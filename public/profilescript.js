...
async function initProfile() {
    try {
        ...
        if (user.company) document.getElementById('form-company').value = user.company;

        // Счетчик активных заявок
        const activeLoadsElement = document.getElementById('active-loads-count');
        if (activeLoadsElement) {
            activeLoadsElement.textContent = user.activeLoads !== undefined ? user.activeLoads : 0;
        }
...
async function loadMyCargo() {
    ...
    cargoContainer.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">Загрузка...</p>';
    ...
    try {
        ...
        if (myCargo && myCargo.length > 0) {
            cargoContainer.innerHTML = myCargo.map(cargo => `
                <div class="cargo-item" style="border: 1px solid #eee; margin-bottom: 15px; padding: 15px; border-radius: 12px; display: flex; justify-content: space-between; align-items: center; background: #fff;">
                    <div class="cargo-info">
                        ...
                        <div style="font-size: 13px; color: #666;">
                            ${cargo.type || 'Груз'} • ${cargo.weight || '?'} т. • ${cargo.volume || '?'} м³
                        </div>
                        <div style="font-size: 11px; color: #999; margin-top: 5px;">Дата: ${cargo.date || 'Не указана'}</div>
                    </div>
                    <div style="display: flex; gap: 8px;">
                        <button onclick="trackCargo(${cargo.id})" style="background: #e1f5fe; border: 1px solid #b3e5fc; color: #0288d1; padding: 5px 12px; border-radius: 6px; cursor: pointer; font-size: 12px; font-weight: 600;">Отследить</button>
                        <button onclick="deleteCargo(${cargo.id})" style="background: #fff5f5; border: 1px solid #ffcccc; color: #ff4d4d; padding: 5px 12px; border-radius: 6px; cursor: pointer; font-size: 12px;">Удалить</button>
                    </div>
                </div>
            `).join('');
        } else {
            cargoContainer.innerHTML = '<p style="text-align: center; color: #666; padding: 40px 0;">У вас пока нет активных грузов.</p>';
        }
    } catch (err) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const addCargoBtn = document.querySelector('#tab-cargo .btn-save');
    if (addCargoBtn) {
        addCargoBtn.onclick = () => { window.location.href = 'page4/index.html'; };
    }
